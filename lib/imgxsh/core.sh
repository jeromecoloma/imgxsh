...
imgxsh::check_dependencies() {
  local missing_deps=()
  local required_commands=("convert" "identify" "pdfimages" "unzip" "yq")
...
imgxsh::validate_config() {
  ...
  # yq is required for configuration parsing/validation
  ...
  if yq eval '.settings' "$config_file" >/dev/null 2>&1; then
    ...
    log::error "Missing required section: settings"
    ((error_count++))
  fi
  ...
  if command -v yq >/dev/null 2>&1; then
    ...
    case "$key" in
      ...
      "settings.parallel_jobs")
        ...
        ;;
      *)
        # For unsupported keys without yq, return empty result but do not fail
        # to avoid breaking callers under 'set -e'.
        return 0
        ;;
    esac
  fi
